...
public class Player_2_Movement : MonoBehaviour
{
    ...
    public static float enter_time;
    public static bool arcade_check = false;
    ...
    void FixedUpdate()
    {
        ...
        if (horizontal > 0.5f && can_move && !attacking)
        {
            transform.localScale = new Vector3(scale.x, scale.y, scale.z);
            anim.SetInteger("horizontal", 1);
            facing_right = true;
            anim.SetBool("drink", false);
            if (!audi[0].isPlaying && grounded)
            {
                audi[0].volume = Random.Range(0.5f, 1);
                audi[0].pitch = Random.Range(0.5f, 1);
                audi[0].Play();
            }
        }

        else if (horizontal < -0.5f && can_move && !attacking)
        {
            transform.localScale = new Vector3(-scale.x, scale.y, scale.z);
            anim.SetInteger("horizontal", -1);
            facing_right = false;
            ...
            if (!audi[0].isPlaying && grounded)
            {
                audi[0].volume = Random.Range(0.7f, 1);
                audi[0].pitch = Random.Range(0.5f, 1);
                audi[0].Play();
            }
        }
        ...
        //if (!jumping && myRigid.velocity.x > max_speed)
        //{
        //    myRigid.velocity = new Vector2(max_speed, myRigid.velocity.y);
        //}
        ...
        //if (jumping && myRigid.velocity.x > max_speed)
        //{
        //    myRigid.velocity = new Vector2(max_speed/2, myRigid.velocity.y);
        //}
    ...
    void OnTriggerStay2D(Collider2D col)
    {
        ...
        if (col.gameObject.name == "Slot_Machine_1")
        {
            if (Input.GetButtonDown("Trigger_2"))
            {
                StartCoroutine(Hacking_1());
            }
        }
        if (col.gameObject.name == "Slot_Machine_2")
        {
            if (Input.GetButtonDown("Trigger_2"))
            {
                StartCoroutine(Hacking_2());
            }
        }
